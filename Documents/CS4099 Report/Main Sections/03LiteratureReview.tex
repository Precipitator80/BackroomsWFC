...
Another issue identified with \acrshort{wfc} is performance. While the chance of success is high with small inputs, larger inputs are much likelier to fail, especially with more complicated tile sets \cite{WFC_ConstraintSolving_and_ML}. This can result in a significant generation time for large outputs. Several solutions to \acrshort{wfc}'s scalability problem have been proposed.%(More citations and graph of performance)
...
\begin{figure}[H]
    ...
    \hfill
    \subfigure[Fitting \acrshort{wfc} onto an irregular quadrilateral grid]{
